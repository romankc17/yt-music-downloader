...
class Window(tk.Tk):
    def __init__(self,*args,**kwargs):
        ...
        tk.Tk.wm_title(self, "Yotube Music Downloader")
...
class StartPage(ttk.Frame):
    ...
    def playbuttons(self):
        frame = tk.Frame(self.container, bg='#66ff99', pady=1, padx=25)
        imBtn = ttk.Button(frame, cursor='plus', style='image.TButton',
                           command=lambda song=(PATH + '/' + 'DJ Snake, Lauv - A Different Way (Official Video).mp3'): self.play(song))
        imBtn.pack()
        image = resize_image('images/music_logo.png', 60, 60)
        imBtn.config(image=image)
        imBtn.photo = image
        frame.pack(fill='x', side=tk.BOTTOM, expand=True)
        # back_btn = Photo
        # forward_btn =
        # play_btn =
        # pause_btn =
    ...
    def canvasFrame(self):
        ...
        for index, file in enumerate(files_list):
            ...
            #displaying songs
            if t.title and t.artist: #only display the song which has title and artist name
